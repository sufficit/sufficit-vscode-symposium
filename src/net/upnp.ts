...
export function discoverUpnpGateway(timeoutMs = 3000): Promise<UpnpDevice | null> {
    return new Promise((resolve) => {
        ...
        sock.on("message", (msg) => {
            ...
            if (loc) {
                done({ descriptionUrl: loc[1].trim(), usn: usn ? usn[1].trim() : "" });
            }
        });
        sock.on("error", () => done(null));
        sock.bind(() => {
            sock.setBroadcast(true);
            sock.send(message, SSDP_PORT, SSDP_ADDR);
        });
...
export async function parseDeviceDescription(descriptionUrl: string): Promise<ParsedDevice | null> {
    try {
        ...
        while ((m = re.exec(xml)) !== null) {
            ...
            if (stM && cuM) {
                services.push({ st: stM[1].trim(), cu: cuM[1].trim() });
            }
        }
        const wan =
            services.find((s) => s.st.includes("WANIPConnection")) ??
            services.find((s) => s.st.includes("WANPPPConnection"));
        if (!wan) {
            return null;
        }
        let cu = wan.cu;
...
export async function addPortMapping(
    ...
    internalPort: number,
    externalPort?: number,
    description = "Symposium Remote Bridge",
): Promise<UpnpPortMapping | null> {
    ...
    try {
        const res = await fetch(parsed.controlUrl, {
            method: "POST",
            headers: {
                "content-type": "text/xml",
                soapaction: parsed.serviceType + "#AddPortMapping",
            },
            body,
        });
        if (!res.ok) {
...
export async function removePortMapping(
    ...
    try {
        const res = await fetch(parsed.controlUrl, {
            ...
            headers: {
                "content-type": "text/xml",
                soapaction: parsed.serviceType + "#DeletePortMapping",
            },
            body,
        });
        return res.ok;
