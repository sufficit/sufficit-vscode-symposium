...
export function importAgents(): { created: number; skipped: number } {
    ...
    for (const dir of [
        ...
        for (const f of files) {
            try {
                ...
                if (parsed) {
                    write(toAgentDef(parsed, f.replace(/\.md$/, "")));
                }
            } catch {
                /* skip unreadable */
            }
    ...
    let dirs: fs.Dirent[];
    try {
        dirs = fs.readdirSync(codexSkills, { withFileTypes: true });
    } catch {
        dirs = [];
    }
    for (const e of dirs) {
        ...
        try {
            const parsed = parse(
                fs.readFileSync(path.join(codexSkills, e.name, "SKILL.md"), "utf8"),
            );
            if (parsed) {
                write(toAgentDef(parsed, e.name));
            }
        } catch {
            /* skip */
        }
