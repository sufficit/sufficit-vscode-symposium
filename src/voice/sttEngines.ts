...
function tmpFile(ext: string): string {
    return path.join(
        os.tmpdir(),
        `symposium-stt-${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`,
    );
}
...
export async function toWav16k(inputPath: string, ffmpegPath: string): Promise<string> {
    ...
    if (r.code !== 0 || !fs.existsSync(out)) {
        throw new Error(
            `ffmpeg conversion failed (code ${r.code}). ${r.stderr.split("\n").slice(-3).join(" ").trim()}`,
        );
    }
    return out;
}
...
export async function transcribeWhisperCpp(wavPath: string, o: WhisperCppOptions): Promise<string> {
    if (!fs.existsSync(o.modelPath)) {
        throw new Error(
            "whisper.cpp model not downloaded. Pick a model in Settings → Voice and download it.",
        );
    }
    const args = [
        "-m",
    ...
    if (o.initialPrompt && o.initialPrompt.trim()) {
        args.push("--prompt", o.initialPrompt.trim());
    }
    const r = await run(o.binary || "whisper-cli", args);
    if (r.code !== 0) {
        throw new Error(
            `whisper-cli failed (code ${r.code}). ${r.stderr.split("\n").slice(-3).join(" ").trim()}`,
        );
    }
...
export async function transcribeFasterWhisper(
    ...
    if (o.language && o.language !== "auto") {
        args.push("--language", o.language);
    }
    try {
        const r = await run(o.binary || "whisper-ctranslate2", args);
        if (r.code !== 0) {
            throw new Error(
                `whisper-ctranslate2 failed (code ${r.code}). ${r.stderr.split("\n").slice(-3).join(" ").trim()}`,
            );
        }
...
export async function transcribeVosk(wavPath: string, o: VoskOptions): Promise<string> {
    if (!fs.existsSync(o.modelPath)) {
        throw new Error(
            "Vosk model not downloaded. Pick a model in Settings → Voice and download it.",
        );
    }
    ...
    if (r.code !== 0) {
        throw new Error(
            `vosk-transcriber failed (code ${r.code}). Install with 'pip install vosk'. ${r.stderr.split("\n").slice(-3).join(" ").trim()}`,
        );
    }
