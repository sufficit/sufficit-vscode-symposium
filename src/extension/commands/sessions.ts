...
export function registerSessionCommands(ctx: CommandContext): void {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand("symposium.pickEditorSession", async () => {
            ...
            if (choice) {
                ChatPanel.openSession(context, surfaceDeps, choice.info);
            }
        }),
        ...
        vscode.commands.registerCommand(
            "symposium.renameSession",
            async (item: { info?: SessionInfo } | SessionInfo) => {
            ...
            async (
                ...
                if (!adapter?.deleteSession) {
                    ...
                    const pick = await vscode.window.showWarningMessage(
                        `Deleting ${info.backend} sessions is not supported.`,
                        "Copy details",
                    );
                ...
                try {
                    ...
                    let expired = 0;
                    try {
                        expired = await expireSessionTasks(new HubClient(), info.sessionId);
                    } catch {
                        /* best-effort */
                    }
                    if (expired) {
                        symposiumLog(
                            `[delete] expired ${expired} memory task(s) for ${info.sessionId}`,
                        );
                    }
                    const residualList =
                        Array.isArray(residual) && residual.length ? residual : undefined;
                    if (!silent) {
                        if (residualList) {
                            ...
                            void vscode.window.showInformationMessage(
                                `Session "${info.title}" permanently deleted.`,
                            );
                        }
                    }
