...
export async function consumeStream(
    ...
    const done = () => {
        ...
        return {
            ...
            aborted: false,
            usage: usage ? stampUsage(usage) : undefined,
        };
    };
    try {
        for (;;) {
                ...
                try {
                    ...
                    if (responses) {
                        ...
                        if (ty === "response.output_text.delta" && typeof json.delta === "string") {
                            ...
                            if (calls[i]) {
                                calls[i].function.arguments += json.delta;
                            }
                        } else if (
                            ...
                            const i = json.output_index ?? lastFnIndex;
                            if (calls[i]) {
                                calls[i].function.arguments = json.arguments;
                            }
                        } else if (ty === "response.error") {
                            cb.onError(String(json?.error?.message ?? "response error"));
                        } else if (
                            (ty === "response.completed" || ty === "response.incomplete") &&
                            json?.response?.usage
                        ) {
                            ...
                            usage = {
                                ...
                                model:
                                    stringOrUndefined(meta.effective_model_id) ||
                                    stringOrUndefined(json.response.model) ||
                                    effectiveModel,
