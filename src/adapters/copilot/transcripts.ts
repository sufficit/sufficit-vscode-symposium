...
function chatSessionTitle(file: string): string | undefined {
    try {
        ...
        for (const line of lines) {
            ...
            let j: {
                kind: number;
                v?: { inputText?: string; inputState?: { inputText?: string } };
                k?: [string, string];
            };
            try {
                j = JSON.parse(line);
            ...
            if (j && j.kind === 0 && j.v) {
                const t =
                    typeof j.v.inputState?.inputText === "string"
                        ? j.v.inputState.inputText.trim()
                        : "";
                if (t) {
                    inputText = t;
                }
            ...
            if (j && j.kind === 1 && Array.isArray(j.k)) {
                if (
                    ...
                    if (vTrimmed && vTrimmed.length < 200 && /^[a-f0-9]+$/i.test(vTrimmed)) {
                        continue;
                    }
                    if (vTrimmed) {
                        inputText = vTrimmed;
...
export function allCopilotSessions(): Map<
    string,
    { label: string; updatedTs: number; isTranscript: boolean }
> {
