...
test("parseCodexUsage: turn.completed with usage (no context window)", () => {
    const ev = {
        type: "turn.completed",
        usage: { input_tokens: 5000, cached_input_tokens: 1200, output_tokens: 300 },
    };
    assert.deepEqual(parseCodexUsage(ev), {
        inputTokens: 5000,
        outputTokens: 300,
...
test("parseCodexUsage: input_tokens already includes cached (not double-counted)", () => {
    ...
    const ev = {
        type: "turn.completed",
        usage: { input_tokens: 73659, cached_input_tokens: 73600, output_tokens: 10 },
    };
...
test("parseCodexUsage: undefined when no usage present", () => {
    ...
    assert.equal(
        parseCodexUsage({ info: { last_token_usage: { input_tokens: 0, output_tokens: 0 } } }),
        undefined,
    );
});
...
test("diffCounts/editDiff: native edit_file / write_file tools tracked like Edit/Write", () => {
    ...
    assert.deepEqual(editDiff("edit_file", { old_string: "a", new_string: "b" }), [
        { old: "a", new: "b" },
    ]);
});

test("editDiff: hunks per tool", () => {
    assert.deepEqual(editDiff("Edit", { old_string: "x", new_string: "y" }), [
        { old: "x", new: "y" },
    ]);
