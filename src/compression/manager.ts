...
export class CompressionManager {
    ...
    private constructor() {
        ...
        vscode.workspace.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration("symposium.compression")) {
                ...
                symposiumLog.info("Configurações de compressão recarregadas");
            }
        });
    }
    ...
    private loadPresetsFromConfig(): void {
        ...
        symposiumLog.info(`CompressionManager carregado: ${this.presets.size} presets, padrão: ${this.settings.defaultPresetId}`);
    }
    ...
    savePreset(preset: CompressionPreset): void {
        ...
        this.changeEmitter.fire();
        symposiumLog.info(`Preset "${preset.id}" salvo`);
    }
    ...
    removePreset(id: string): void {
        ...
        this.changeEmitter.fire();
        symposiumLog.info(`Preset "${id}" removido`);
    }
    ...
    setSectionConfig(sessionId: string, presetId: string, override: boolean = true): void {
        ...
        symposiumLog.info(`Seção ${sessionId}: compressão definida para "${presetId}"`);
    }
    ...
    removeSectionConfig(sessionId: string): void {
        ...
        symposiumLog.info(`Seção ${sessionId}: configuração de compressão removida`);
    }
    ...
    async importSettings(json: string): Promise<void> {
        try {
            ...
            this.loadPresetsFromConfig();
            this.changeEmitter.fire();
            symposiumLog.info("Configurações de compressão importadas com sucesso");
        } catch (error) {
            symposiumLog.error("Erro ao importar configurações de compressão:", error);
            throw error;
        }
    }
    ...
    async applyCompression(
        ...
        sessionContext?: { sessionId: string; model?: string }
        ...
        switch (preset.strategy) {
            ...
            case "summarize":
                ...
                symposiumLog.warn("Estratégia 'summarize' ainda não implementada, usando 'prune-old' como fallback");
    ...
    private pruneOld(messages: any[], preset: CompressionPreset): any[] {
        ...
        const systemMessages = messages.filter((m, i) => 
            m.role === "system" && (preset.preserveSystemMessages !== false)
        );

        // Mensagens não-sistema
        const otherMessages = messages.filter((m, i) => m.role !== "system" || !preset.preserveSystemMessages);
    ...
    private foldTokens(messages: any[], preset: CompressionPreset): any[] {
        ...
        const systemMessages = messageTokens.filter(
            (mt, i) => mt.msg.role === "system" && preset.preserveSystemMessages !== false
        ).map(mt => mt.msg);
