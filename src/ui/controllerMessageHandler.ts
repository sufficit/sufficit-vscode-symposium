...
export async function handleControllerMessage(message: WebviewToHost, ctx: ControllerMessageContext): Promise<boolean> {
    switch (message.type) {
        case "send":
            ctx.onSend(
                { clientMessageId: message.clientMessageId, intentId: message.intentId, text: message.text, attachments: message.attachments ?? [], model: message.model, reasoning: message.reasoning, permission: message.permission, autonomy: message.autonomy, interruptedBy: message.interruptedBy },
