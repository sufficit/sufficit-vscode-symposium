...
export function applyStaticI18n(): void {
    ...
    if (sessionSearch) {
        sessionSearch.placeholder = t("sessions.search.placeholder");
        sessionSearch.setAttribute("aria-label", t("sessions.search.aria"));
    }

    const sh = document.querySelector("#sessionsHeader span");
    if (sh) {
        sh.textContent = t("chat.sessions.label");
    }
    const af = document.getElementById("accountFooter");
    if (af) {
        af.title = t("chat.account.tooltip");
    }
    const rz = document.getElementById("resizer");
    if (rz) {
        rz.title = t("chat.resizer.tooltip");
    }

    setT(listToggle, "chat.icon.toggleSessions");
    setT(switchAgentBtn, "chat.icon.switchAgent");
    if (copySessionBtn) {
        copySessionBtn.title = t("chat.icon.copySession.title");
        copySessionBtn.setAttribute("aria-label", t("chat.icon.copySession.aria"));
    }
    ...
    if (modelPicker) {
        modelPicker.title = t("chat.picker.model.tooltip");
        modelPicker.setAttribute("aria-label", t("chat.picker.model.tooltip"));
    }
    if (reasoningPicker) {
        reasoningPicker.title = t("chat.picker.reasoning.tooltip");
        reasoningPicker.setAttribute("aria-label", t("chat.picker.reasoning.aria"));
    }
    if (presencePicker) {
        presencePicker.title = t("chat.picker.presence.tooltip");
        presencePicker.setAttribute("aria-label", t("chat.picker.presence.tooltip"));
    }
    const ep = document.getElementById("execPicker");
    if (ep) {
        ep.title = t("chat.picker.exec.tooltip");
        ep.setAttribute("aria-label", t("chat.picker.exec.tooltip"));
    }
    ...
    if (stopBtn) {
        stopBtn.title = t("chat.icon.stop.title");
        stopBtn.setAttribute("aria-label", t("chat.icon.stop.aria"));
    }
    ...
    if (cancelEdit) {
        ...
        if (label) {
            label.textContent = t("chat.icon.cancelEdit.label");
        }
    }
